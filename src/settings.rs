...
impl Settings {
    ...
    pub fn add_job_file(&mut self, p: &Path) -> Result<(), String> {
        if p.exists() && p.is_file() {
            self.job_files.push(String::from(p.to_str().unwrap()));
            return Ok(());
        } else {
            return Err(format!("Path: {} either is not valid or is not a file", p.to_str().unwrap()));
        }
    ...
    pub fn remove_job_file(&mut self, i: usize) -> Result<(), String> {
        if i < self.job_files.len() {
            self.job_files.swap_remove(i);
            return Ok(());
        } else {
            return Err(format!("Cannot remove job file. Index out of bounds"));
        }
    }

    pub fn get_job_file(&self, i: usize) -> Result<String, String> {
        if i < self.job_files.len() {
            Ok(self.job_files.get(i).unwrap().clone())
        } else {
            return Err(format!("Cannot get job file. Index out of bounds"));
        }
    }

    pub fn get_all_job_files(&self) -> Vec<String> {
        self.job_files.clone()
    }
}